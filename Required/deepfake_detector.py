...
class DeepfakeDetector:
    ...
    def __init__(self):
        # Slightly stricter defaults help catch modern generated videos
        # that look smooth frame-to-frame but still have subtle instability.
        self.temporal_threshold = 0.12  # Max allowed frame-to-frame change
        self.artifact_threshold = 26    # Gradient threshold for artifacts
        self.blink_min_frames = 2       # Minimum frames for valid blink
        self.blink_max_frames = 8       # Maximum frames for valid blink
        self.deepfake_threshold = 58.0
        self.suspicious_threshold = 76.0

    def analyze_texture_naturalness(self, frames, face_bboxes):
        """
        Analyze face texture sharpness. Many generated videos are over-smoothed,
        resulting in low Laplacian variance in face regions.
        """
        lap_vars = []
        ...
        median_lap = float(np.median(lap_vars))
        # Empirical normalization: low variance -> more suspicious.
        texture_score = float(np.clip((median_lap / 120.0) * 100.0, 0.0, 100.0))
        return texture_score, median_lap
    ...
    def detect_deepfake(self, predictions_sequence, frames, face_bboxes):
        ...
        # 5. Texture Naturalness
        texture_score, median_lap = self.analyze_texture_naturalness(frames, face_bboxes)
        report['details']['texture_naturalness'] = {
            'score': texture_score,
            'median_laplacian_variance': float(median_lap)
        }

        if median_lap > 0 and median_lap < 55:
            report['warnings'].append(f"Over-smoothed face texture detected (lap_var: {median_lap:.2f})")
        ...
        # Count suspicion cues so generated content can be flagged earlier.
        suspicion_cues = 0
        if len(inconsistencies) > max(6, int(len(predictions_sequence) * 0.08)):
            suspicion_cues += 1
        if avg_artifacts > 10:
            suspicion_cues += 1
        if len(blinks) < max(1, int(expected_blinks * 0.45)):
            suspicion_cues += 1
        if avg_stability > 35:
            suspicion_cues += 1
        if median_lap > 0 and median_lap < 55:
        ...
        report['details']['suspicion_cues'] = int(suspicion_cues)

        # Determine authenticity using stricter thresholds.
        # Conservative shortcut: require more corroborating cues before forcing fake.
        if overall_score < self.deepfake_threshold or (suspicion_cues >= 3 and overall_score < 70.0):
