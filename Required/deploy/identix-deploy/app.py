...
from fresh_deepfake_pipeline import FreshDeepfakePipeline

CURRENT_DIR = Path(__file__).resolve().parent
SHARED_ROOT = CURRENT_DIR.parent.parent
...
print("Initializing fresh deepfake pipeline...")
deepfake_pipeline = FreshDeepfakePipeline()
DEEPFAKE_MODEL_VERSION = deepfake_pipeline.model_version
print(f"✓ {DEEPFAKE_MODEL_VERSION} initialized")

deepfake_v2_detector = None
deepfake_v2_load_error = None
deepfake_v3_detector = None
deepfake_v3_load_error = None
deepfake_v4_detector = None
deepfake_v4_load_error = None

V2_FAKE_THRESHOLD = float(os.environ.get('V2_FAKE_THRESHOLD', '0.48'))
...
V2_DECISION_CONFIDENCE_MIN = float(os.environ.get('V2_DECISION_CONFIDENCE_MIN', '0.60'))
...
V3_V4_THRESHOLD = 0.5
V3_DEEPFAKE_THRESHOLD = float(os.environ.get('V3_DEEPFAKE_THRESHOLD', '0.80'))
V3_AUTHENTIC_THRESHOLD = float(os.environ.get('V3_AUTHENTIC_THRESHOLD', '0.30'))
V4_DEEPFAKE_THRESHOLD = float(os.environ.get('V4_DEEPFAKE_THRESHOLD', '0.80'))
V4_AUTHENTIC_THRESHOLD = float(os.environ.get('V4_AUTHENTIC_THRESHOLD', '0.35'))

def get_deepfake_v2_detector():
    """Load v2 model lazily so v1 startup behavior stays unchanged."""
    global deepfake_v2_detector, deepfake_v2_load_error

    if deepfake_v2_detector is not None:
        return deepfake_v2_detector
    ...
    model_candidates = [
        Path(r"D:\link2\Capstone 4-1\Code_try_1\Required\models\deepfake_detector_v2_ffa_mpdv.pth"),
        Path("models/deepfake_detector_v2_ffa_mpdv.pth"),
        Path("../models/deepfake_detector_v2_ffa_mpdv.pth"),
    ]

    model_path = next((p for p in model_candidates if p.exists()), None)
    if model_path is None:
        deepfake_v2_load_error = "v2 model file not found (deepfake_detector_v2_ffa_mpdv.pth)"
        raise FileNotFoundError(deepfake_v2_load_error)

    runtime_device = 'cuda' if device.type == 'cuda' else 'cpu'
    deepfake_v2_detector = load_v2_model(str(model_path), device=runtime_device)
    print(f"✓ Version 2 FFA-MPDV Deepfake Detector initialized from {model_path} ({runtime_device})")
    return deepfake_v2_detector


def get_deepfake_v3_detector():
    """Load DeepfakeBench Meso4 v3 lazily."""
    global deepfake_v3_detector, deepfake_v3_load_error

    if deepfake_v3_detector is not None:
        return deepfake_v3_detector
    ...
    model_candidates = [
        Path(r"D:\link2\Capstone 4-1\Code_try_1\Required\models\deepfakebench_meso4_best.pth"),
        Path("models/deepfakebench_meso4_best.pth"),
        Path("../models/deepfakebench_meso4_best.pth"),
    ]

    model_path = next((p for p in model_candidates if p.exists()), None)
    if model_path is None:
        deepfake_v3_load_error = "v3 model file not found (deepfakebench_meso4_best.pth)"
        raise FileNotFoundError(deepfake_v3_load_error)

    runtime_device = 'cuda' if device.type == 'cuda' else 'cpu'
    deepfake_v3_detector = load_v3_model(str(model_path), device=runtime_device)
    print(f"✓ Version 3 DeepfakeBench Meso4 Detector initialized from {model_path} ({runtime_device})")
    return deepfake_v3_detector


def get_deepfake_v4_detector():
    """Load Selim DFDC B7 v4 lazily."""
    global deepfake_v4_detector, deepfake_v4_load_error

    if deepfake_v4_detector is not None:
        return deepfake_v4_detector
    ...
    model_candidates = [
        Path(r"D:\link2\Capstone 4-1\Code_try_1\Required\models\selimsef_b7_final_111.pth"),
        Path("models/selimsef_b7_final_111.pth"),
        Path("../models/selimsef_b7_final_111.pth"),
    ]

    model_path = next((p for p in model_candidates if p.exists()), None)
    if model_path is None:
        deepfake_v4_load_error = "v4 model file not found (selimsef_b7_final_111.pth)"
        raise FileNotFoundError(deepfake_v4_load_error)

    runtime_device = 'cuda' if device.type == 'cuda' else 'cpu'
    deepfake_v4_detector = load_v4_model(str(model_path), device=runtime_device)
    print(f"✓ Version 4 Selim DFDC EfficientNet-B7 Detector initialized from {model_path} ({runtime_device})")
    return deepfake_v4_detector
...
def summarize_v2_predictions(frame_predictions):
    ...
    if total_weight > 0:
        ...
        weighted_fake_score = float(np.mean([float(p.get('probability_fake', 0.0)) for p in frame_predictions]))
    ...
    margin = abs(weighted_fake_score - V2_FAKE_THRESHOLD)
    margin_conf = min(1.0, margin / max(1e-6, (0.5 - V2_UNCERTAIN_BAND)))
    fused_conf = max(0.0, min(1.0, 0.65 * margin_conf + 0.35 * mean_conf))

    deepfake_gate = weighted_fake_score >= (V2_FAKE_THRESHOLD + V2_DEEPFAKE_MARGIN)
    authentic_gate = weighted_fake_score <= (V2_FAKE_THRESHOLD - V2_AUTHENTIC_MARGIN)

    if fused_conf < 0.22 or usable_frame_ratio < 0.35:
        verdict = 'LOW_CONFIDENCE_REVIEW'
    ...
    if verdict == 'LOW_CONFIDENCE_REVIEW':
...
def summarize_v3_deepfakebench(frame_predictions):
    ...
    score = float(np.mean(sampled)) if sampled else 0.5
...
def summarize_v4_selim(frame_predictions):
    probs = [float(p.get('probability_fake', 0.0)) for p in frame_predictions]
    sampled = _uniform_sample(probs, V3_V4_FRAME_COUNT)
    score = _selim_confident_strategy(sampled, t=0.8) if sampled else 0.5
...
def get_center_fallback_bbox(image_size, scale=0.62):
    """Return a centered square bbox as a fallback when no face is found."""
...
def find_main_face_bbox(frames, search_limit=12):
    """Scan early frames for a usable main face; fallback to center crop if needed."""
    if not frames:
        return None, -1, True
...
def extract_frames(video_path, max_frames=100):
    ...
    for idx in frame_indices:
        ...
        if ret and frame is not None:
            # Keep memory bounded for long/high-res videos to avoid request drops.
            h, w = frame.shape[:2]
            max_side = 960
            largest = max(h, w)
            if largest > max_side:
...
def detect_deepfake():
    """Analyze video for deepfake detection with the fresh pipeline."""
    try:
        # Check if video was uploaded
        if 'video' not in request.files:
            return jsonify({'error': 'No video uploaded'}), 400
        ...
        # Get max_frames parameter and keep it in a safe range.
        max_frames_raw = int(request.form.get('max_frames', 100))
        max_frames = max(20, min(max_frames_raw, 200))
        model_key = request.form.get('model_key', 'identix_v1')
        ...
        try:
            ...
            # Find initial face across early frames; fallback to center crop if needed.
            ...
            if model_key == 'ffa_mpdv_v2':
                detector_v2 = get_deepfake_v2_detector()
                frame_predictions = []
                valid_frames = 0
                ...
                for frame, bbox in zip(frames_with_face, tracked_bboxes):
                    ...
                    x1 = max(0, x - pad)
                    y1 = max(0, y - pad)
                    x2 = min(frame.size[0], x + w + pad)
                    y2 = min(frame.size[1], y + h + pad)
                ...
                if valid_frames == 0:
                    return jsonify({'error': 'Could not extract valid face crops for v2 detection'}), 400
            ...
            elif model_key == 'deepfakebench_meso4_v3':
                detector_v3 = get_deepfake_v3_detector()
                frame_predictions = []
                valid_frames = 0
                ...
                if valid_frames == 0:
                    return jsonify({'error': 'Could not extract valid face crops for v3 detection'}), 400
            ...
            elif model_key == 'selim_b7_v4':
                detector_v4 = get_deepfake_v4_detector()
                frame_predictions = []
                valid_frames = 0
                ...
                if valid_frames == 0:
                    return jsonify({'error': 'Could not extract valid face crops for v4 detection'}), 400
                ...
                deepfake_report = deepfake_pipeline.analyze_video_frames(frames_with_face, tracked_bboxes)
                deepfake_report['model_version'] = DEEPFAKE_MODEL_VERSION
                deepfake_report['seed_face_frame_index'] = int(face_seed_frame)
                deepfake_report['used_center_fallback_bbox'] = bool(used_fallback_bbox)
            
            # Add metadata
            deepfake_report['video_info'] = {
                'total_frames': int(total_frames),
                'analyzed_frames': len(frames_with_face),
                'fps': float(fps) if fps else None
            }
            
            return jsonify({
                'success': True,
                'model_version': deepfake_report.get('model_version', DEEPFAKE_MODEL_VERSION),
...
def detect_deepfake_image():
    """Analyze a single image using the fresh deepfake pipeline."""
    try:
        if 'image' not in request.files:
            return jsonify({'error': 'No image uploaded'}), 400
        ...
        if model_key == 'ffa_mpdv_v2':
            ...
            if faces:
                ...
                x1 = max(0, x - pad)
                y1 = max(0, y - pad)
                x2 = min(pil_img.size[0], x + w + pad)
                y2 = min(pil_img.size[1], y + h + pad)
                if x2 > x1 and y2 > y1:
                    pil_img = pil_img.crop((x1, y1, x2, y2))

            pred = detector_v2.predict(pil_img, return_proba=True, threshold=V2_FAKE_THRESHOLD)
            fake_prob = float(pred.get('proba', 0.0))
            model_conf = float(pred.get('confidence', 0.0))
            margin = abs(fake_prob - V2_FAKE_THRESHOLD)
            margin_conf = min(1.0, margin / max(1e-6, (0.5 - V2_UNCERTAIN_BAND)))
            fused_conf = max(0.0, min(1.0, 0.65 * margin_conf + 0.35 * model_conf))

            if fused_conf < 0.22:
                verdict = 'LOW_CONFIDENCE_REVIEW'
            elif fake_prob >= V2_FAKE_THRESHOLD:
                verdict = 'LIKELY_DEEPFAKE'
            else:
                verdict = 'LIKELY_AUTHENTIC'
            ...
            report = {
                'verdict': verdict,
                'confidence': float(max(1.0, min(99.0, fused_conf * 100.0))),
                'prediction': {
                    'class': 'FAKE' if fake_prob >= V2_FAKE_THRESHOLD else 'REAL',
                    'confidence': float(max(1.0, min(99.0, fused_conf * 100.0))),
            ...
            return jsonify({'success': True, 'model_version': report['model_version'], 'report': report})
        ...
        if model_key == 'deepfakebench_meso4_v3':
            ...
            pred = detector_v3.predict(pil_img, return_proba=True, threshold=V3_V4_THRESHOLD)
            fake_prob = float(pred.get('proba', 0.0))
            verdict = 'LIKELY_DEEPFAKE' if fake_prob >= V3_V4_THRESHOLD else 'LIKELY_AUTHENTIC'
            confidence_pct = float(max(1.0, min(99.0, abs(fake_prob - V3_V4_THRESHOLD) * 2.0 * 100.0)))

            report = {
                'verdict': verdict,
                'confidence': confidence_pct,
                'prediction': {
                    'class': 'FAKE' if fake_prob >= V3_V4_THRESHOLD else 'REAL',
                    'confidence': confidence_pct,
                    'probability_fake': fake_prob,
                    'decision_threshold': float(V3_V4_THRESHOLD),
                    'source_project': 'SCLBD/DeepfakeBench',
                },
                'model_version': 'Version 3 DeepfakeBench Meso4 Detector',
            }
        ...
        if model_key == 'selim_b7_v4':
            ...
            pred = detector_v4.predict(pil_img, return_proba=True, threshold=V3_V4_THRESHOLD)
            fake_prob = float(pred.get('proba', 0.0))
            verdict = 'LIKELY_DEEPFAKE' if fake_prob >= V3_V4_THRESHOLD else 'LIKELY_AUTHENTIC'
            confidence_pct = float(max(1.0, min(99.0, abs(fake_prob - V3_V4_THRESHOLD) * 2.0 * 100.0)))
        ...
        result, err = deepfake_pipeline.analyze_image(image_bgr, detect_faces)
        if err is not None:
            status = 503 if 'not loaded' in err else 400
            return jsonify({'error': err, 'model_version': DEEPFAKE_MODEL_VERSION}), status

        result['model_version'] = DEEPFAKE_MODEL_VERSION
        return jsonify({'success': True, 'model_version': DEEPFAKE_MODEL_VERSION, 'report': result})
